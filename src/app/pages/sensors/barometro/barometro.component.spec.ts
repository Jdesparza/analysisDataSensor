...
describe('BarometroComponent', () => {
  ...
  it('Guardado correcto de sensor externo Barómetro', () => {
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Barómetro'});

    expect(Object.keys(component.calSensorExterno).length > 0).toBeTrue();
  });

  it('Guardado correcto de sensores Barómetro existentes', () => {
    let sensores = sensoresJson.filter((res: {sensorBarometro: any;}) => res.sensorBarometro.isExists == true);
  ...
  it('Cálculo error de un modelo que tenga Barómetro', () => {
    component.infoSensorSmartphone = ['sensorBarometro', 'Barómetro'];
    let sensores = sensoresJson.filter((res: {sensorBarometro: any;}) => res.sensorBarometro.isExists == true);
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Barómetro'});

    let error = component.errorPercentage(sensores[0]);
    
    expect(error).toEqual(0.6225864651916332);
  });
});
