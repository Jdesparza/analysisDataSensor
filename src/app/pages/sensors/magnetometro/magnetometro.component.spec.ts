...
describe('MagnetometroComponent', () => {
  ...
  it('Guardado correcto de sensor externo Magnetómetro', () => {
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Magnetómetro'});

    expect(Object.keys(component.calSensorExterno).length > 0).toBeTrue();
  });

  it('Guardado correcto de sensores Magnetómetro existentes', () => {
    let sensores = sensoresJson.filter((res: {sensorMagnetometro: any;}) => res.sensorMagnetometro.isExists == true);
  ...
  it('Cálculo error de un modelo que tenga Magnetómetro', () => {
    component.infoSensorSmartphone = ['sensorMagnetometro', 'Magnetómetro'];
    let sensores = sensoresJson.filter((res: {sensorMagnetometro: any;}) => res.sensorMagnetometro.isExists == true);
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Magnetómetro'});
