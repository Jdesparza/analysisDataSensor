...
describe('RitmoCardiacoComponent', () => {
  ...
  it('Guardado correcto de sensor externo Ritmo Cardíaco', () => {
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Ritmo Cardíaco'});

    expect(Object.keys(component.calSensorExterno).length > 0).toBeTrue();
  });

  it('Guardado correcto de sensores Ritmo Cardíaco existentes', () => {
    let sensores = sensoresJson.filter((res: {sensorRitmoCardiaco: any;}) => res.sensorRitmoCardiaco.isExists == true);
  ...
  it('Cálculo error de un modelo que tenga Ritmo Cardíaco', () => {
    component.infoSensorSmartphone = ['sensorRitmoCardiaco', 'Ritmo Cardíaco'];
    let sensores = sensoresJson.filter((res: {sensorRitmoCardiaco: any;}) => res.sensorRitmoCardiaco.isExists == true);
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Ritmo Cardíaco'});

    let error = component.errorPercentage(sensores[0]);
    
    expect(error).toEqual(2.5426015132615545);
  });
});
