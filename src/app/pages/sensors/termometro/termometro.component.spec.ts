...
describe('TermometroComponent', () => {
  ...
  it('Guardado correcto de sensor externo Termómetro', () => {
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Termómetro'});

    expect(Object.keys(component.calSensorExterno).length > 0).toBeTrue();
  });

  it('Guardado correcto de sensores Termómetro existentes', () => {
    let sensores = sensoresJson.filter((res: {sensorTermometro: any;}) => res.sensorTermometro.isExists == true);
  ...
  it('Cálculo error de un modelo que tenga Termómetro', () => {
    component.infoSensorSmartphone = ['sensorTermometro', 'Termómetro'];
    let sensores = sensoresJson.filter((res: {sensorTermometro: any;}) => res.sensorTermometro.isExists == true);
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Termómetro'});
