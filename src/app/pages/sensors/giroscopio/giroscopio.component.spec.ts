...
describe('GiroscopioComponent', () => {
  ...
  it('Guardado correcto de sensor externo Giroscopio', () => {
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Giroscopio'});

    expect(Object.keys(component.calSensorExterno).length > 0).toBeTrue();
  });

  it('Guardado correcto de sensores Giroscopio existentes', () => {
    let sensores = sensoresJson.filter((res: {sensorGiroscopio: any;}) => res.sensorGiroscopio.isExists == true);
  ...
  it('Cálculo error de un modelo que tenga Giroscopio', () => {
    component.infoSensorSmartphone = ['sensorGiroscopio', 'Giroscopio'];
    let sensores = sensoresJson.filter((res: {sensorGiroscopio: any;}) => res.sensorGiroscopio.isExists == true);
    component.calSensorExterno = sensoresExternosJson.find((senExt: { sensor: string; }) => {return senExt.sensor === 'Giroscopio'});

    let error = component.errorPercentage(sensores[0]);
    
    expect(error).toEqual(99.99997672942797);
  });
});
